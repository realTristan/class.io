...
#[actix_web::get("/class/{class_id}/submissions/{bearer}")]
async fn get_user_submissions(
    req: HttpRequest, db: web::Data<Database>, class_id: web::Path<String>, bearer: web::Path<String>
) -> impl Responder {
    // Get the access and authentication tokens from
    // the request headers. These tokens are used to make
    // sure that the incoming request isn't from an abuser.
...
async fn insert_class_submission(
    req: HttpRequest, db: web::Data<Database>, class_id: web::Path<String>, 
    submission_id: web::Path<String>, body: web::Json<SubmissionDataBody>
) -> impl Responder {
    // Get the access and authentication tokens from
    // the request headers. These tokens are used to make
    // sure that the incoming request isn't from an abuser.
    ...
    let r: u64 = db.insert_class_submission(&class_id, &submission_id, &submitter_bearer, &body.data).await;
...
async fn delete_class_submission(
    req: HttpRequest, db: web::Data<Database>, submission_id: web::Path<String>, body: web::Json<SubmissionDataBody>
) -> impl Responder {
    // Get the access and authentication tokens from
    // the request headers. These tokens are used to make
    // sure that the incoming request isn't from an abuser.
    ...
    let r: u64 = db.delete_class_submission(&bearer, &submission_id).await;
    // Return whether more than 0 rows were affected
    return format!("{{\"success\": {}}}", r > 0)
}